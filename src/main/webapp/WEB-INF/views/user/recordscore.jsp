                ...
                <div class="post-content">
                    ...
                    <ul class="pagination">
                        <c:if test="${movieScorePage.number != 0}">
                            <li><a href="/user/record/score?p=${movieScorePage.number-1}">&laquo;</a></li>
                        </c:if>
                        <c:forEach var="i" begin="1" end="${movieScorePage.totalPages}" step="1">
                            <li class='<c:if test="${ movieScorePage.number-i == -1}">active</c:if>'><a href="/user/record/score?p=${(i-1)}">${i}</a></li>
                        </c:forEach>
                        <c:if test="${ movieScorePage.number - movieScorePage.totalPages != -1}">
                            <li><a href="/user/record/score?p=${movieScorePage.number+1}">&raquo;</a></li>
                        </c:if>
                    </ul>
                </div>
